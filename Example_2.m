...
A = [exp(1j*2*pi/n*3*nindex),exp(1j*2*pi/n*5*nindex),exp(1j*2*pi/n*7*nindex),exp(1j*2*pi/n*9*nindex)];         
% the sensing matrix
...
for i=1:length(m_sam)
     ...
     J_sum = zeros(p,p);
     for mc=1:MC
          waitbar( ((i-1)*MC +mc)/(length(m_sam)*MC));
           ...
           P=G'/(G*G')*G;
           J_hat_inv = sigma_w*(inv(A'*P*A)-n*sigma_e^2*((A'*P*A)\theta_C)*((A'*P*A)\theta_C)'...
           ...
           theta_hat = gradient_descent(theta_init,A_hat,z_hat,sigma_e,C,sigma_v);        
           % the estimated signals by the gradient descent algorithm
     ...
     LBmatrix = n/m*crb+n^2*(m-n)*sigma_e^2*sigma_w^2*((A'*A)\theta_C)*((A'*A)\theta_C)'/m...
         /(sigma_w+n*sigma_e^2*((theta_C'/(A'*A))*theta_C+(theta_C.'/conj(A'*A))*conj(theta_C)))...
