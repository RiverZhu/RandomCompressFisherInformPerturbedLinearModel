...
J = [A'*A/sigma_w+sigma_e^2*n*(theta_C*theta_C')/(sigma_w^2),sigma_e^2*n*(theta_C*theta_C.')/(sigma_w^2);...
    sigma_e^2*n*(conj(theta_C)*theta_C')/(sigma_w^2),A'*A/sigma_w+sigma_e^2*n*(conj(theta_C)*theta_C.')/(sigma_w^2)];      % the FIM before compression
...
for k = 1:MC
     ...
     J_hat = [M_P/sigma_w+sigma_e^2*n*(theta_C*theta_C')/(sigma_w^2),sigma_e^2*n*(theta_C*theta_C.')/(sigma_w^2);...
        sigma_e^2*n*(conj(theta_C)*theta_C')/(sigma_w^2),M_P/sigma_w+sigma_e^2*n*(conj(theta_C)*theta_C.')/(sigma_w^2)];     % the FIM after compression
